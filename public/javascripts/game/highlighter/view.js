...
(function () {
  ...
  var MODULE        = 'highlighter',
      moduleObject = angular.module(MODULE);
  moduleObject.factory('HighlighterView', ['Log', 'Util', 'ViewManager', 'DomHelper', ViewFactory]);
...
//robkohr
      ...
      $(this.instructionsEl).find('.answer-choice').click(function () {
        if(!$(this).hasClass('unselectable')){
          view.playSound('click');
          $(this).toggleClass('selected').removeClass('show-correct-and-incorrect-selected');
          view.updateMathJaxStyles($(this));
          $('.incorrect-answer.selected.show-correct-and-incorrect-selected').removeClass('selected');
        }
      });
      if (!startingState) {
        ...
        setTimeout(function() {
          ...
          textEl.style.transform = 'scale(1.0, 1.0)';}, 0);
      }
    ...
    View.prototype.clearAnswers = function(){
      ...
      $('.incorrect-answer.selected').removeClass('selected');
    ...
    View.prototype.showAnswers = function () {
      ...
      $('.answer-choice.correct-answer').addClass('selected').addClass('show-correct-and-incorrect-selected');
      viewGbl.updateMathJaxStyles();
    };
    ...
    View.prototype.getAnswers = function () {
      ...
      $('.answer-choice').each(function(){
        ...
        if(el.hasClass('selected') && el.hasClass('correct-answer')){
          user_correct = true;
        }
        if(!el.hasClass('selected') && !el.hasClass('correct-answer')){
          user_correct = true;
        ...
        //Exclude incorrect answers that have not been selected
        if(el.hasClass('selected') || el.hasClass('correct-answer')){
          answers.push({
            dom_id: el.attr('id'),
            id: Number(el.attr('id').replace('answer-id-', '')),
            content: el.text(),
            is_answer_to_choose: el.hasClass('correct-answer'),
